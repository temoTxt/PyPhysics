...
The "(Z-i) ≡ (Z-ii) everywhere" framing above is correct as a statement about the framework's apparatus (the cutoff has no Z), but it leaves a critical empirical gap unaddressed: **the framework's prediction uses $g_s = -2.00231930$ (the Z=1 free-electron value) at every Z**, while the parallel hydrogenic-ion g-factor scan ([PR #87](https://github.com/temoTxt/PyPhysics/pull/87)) shows measured $g_e^{\rm bound}(Z)$ varies by $\sim 10^{-3}$ across Z due to the QED bound-state $-(Z\alpha)^2/3$ Breit term.

**Concrete effect on #3 fine structure:** the $(g_s/-2)$ enhancement factor used in the 1,028 MHz anomalous correction is taken at the Z=1 free-electron value. At Z=3, the *bound-electron* $g_s$ is closer to $-2.00200$ (from the Schneider 2022 Z=2 datum scaled to Z=3 via $-(Z\alpha)^2/3$), giving an enhancement of ~$+0.100\%$ rather than $+0.116\%$. The framework's prediction is therefore $\sim$ 144 MHz lower than what bound-state QED would predict for Li²⁺'s actual $g_e^{\rm bound}$. This is below the Bethe-estimate precision floor (~few MHz at Z=1, scaled to ~few hundred MHz at Z=3 by $Z^4$), so it doesn't shift the "✅ outcome A at Bethe-estimate floor" verdict — but it does sharpen what that verdict means.

**Reconciliation.** The two branches' findings can be stated consistently:

- **Branch A is the framework's stated position** — the cutoff is Z-trivial, so the framework reproduces leading-Dirac $Z^4$ at Z=3 with the same $(g_s/-2)$ multiplier as Z=1.
- **Branch C is what comparison with high-precision Z=3 data would force** — the framework's prediction uses the wrong $g_s$ at Z=3 (the Z=1 value, ~$10^{-3}$ too large in $|g_s|$). At Bethe-estimate precision the discrepancy is below floor; at sub-MHz precision (which the framework's apparatus can't deliver anyway), Branch A would fail.
- **Same verdict shape as the lepton axis** ([PR #70](https://github.com/temoTxt/PyPhysics/pull/70)): apparatus is X-trivial (A); data requires X-specific inheritance from QED (C). The framework's published structure has no mechanism to capture the Z-dependent bound-state corrections, exactly as on the lepton axis.

**The "weak Z-discriminator" framing is therefore correct *but for a stronger reason than first stated***: it isn't just that the cutoff doesn't engage these observables — it's that the *cutoff itself contains no Z-content the framework can supply*, while the data exhibits Z-dependence that originates entirely outside the framework (in textbook bound-state QED). Closing the gap to a Branch B verdict — a Z-dependent cutoff derived from framework structure — is the open question for [#75](https://github.com/temoTxt/PyPhysics/issues/75).

<!-- TODO: human reviews and fills in — confirms (a) the cross-PR reconciliation framing (Branch A structurally + Branch C operationally is the honest joint verdict), (b) the concrete effect on #3 fine structure ($g_s$-mismatch at Z=3 is below Bethe-estimate floor but sharpens the meaning of the verdict), (c) the parallel to PR #70's lepton-axis verdict, and (d) the link to #75 as the open derivational question. -->
