...
|---|---|---|---|---|---|
| Electron `g_{s}` (Finding 2 itself) | `-2.00231930` | `-2.00231930…` (CODATA) | `0` | matches by construction | `-2.00231930436256(35)` — CODATA 2018, `nist.get_constant` |
| Hydrogen `2P₃/₂–2P₁/₂` (PR C) | `~10\,962` MHz | `10\,969.13(10)` MHz | `~7` MHz | Bethe-estimate floor (recoil + 2-loop) | `10\,969.049(15)` MHz — ASD levels, **theoretical/Ritz**, `nist.get_levels` |
| Hydrogen 1S hyperfine (PR F) | `~1\,420.04` MHz | `1\,420.405\,751\,768(2)` MHz | `~0.4` MHz | sub-leading QED (α/π corrections + recoil + nuclear-structure) | `1\,420.405\,751\,768(2)` MHz — curated, `nist.list_dirac_targets` |
| Helium `^{3}P₀-^{3}P_{1}` (PR I) | `~29\,616.95` MHz | `29\,616.952` MHz | `~kHz` | full QED residual *(provisional — spin-spin/spin-orbit recombination not derived in PR I)* | `29\,616.952` MHz — curated, `nist.list_dirac_targets` |
| Positronium ortho-para (PR I) | `~203\,389` MHz | `203\,389(2)` MHz | matches at Bethe precision | positronium-specific QED *(provisional — absorbing correction not derived in PR I)* | `203\,389(2)` MHz — curated, `nist.list_dirac_targets` |
| Muonium hyperfine (PR I) | `~4\,463.4` MHz | `4\,463.302\,776(51)` MHz | `~0.1` MHz | sub-leading QED | `4\,463.302\,776(51)` MHz — curated, `nist.list_dirac_targets` |

**On the `NIST MCP cross-check` column (provenance, pragmatic per Crocco §1).** This column was populated programmatically via the `precision_data` MCP server (under the `nist.*` namespace) ([issue #90](https://github.com/temoTxt/PyPhysics/issues/90), [`mcp_servers/precision_data_mcp/`](../../../mcp_servers/precision_data_mcp/)) on 2026-05-29; it is a retrieval cross-check, **not** a replacement for the `Measurement` column. Each entry has a distinct provenance, and only one is an independent source:

1. **Electron `g_{s}`** — the only entry from an authoritative independent reference: CODATA-2018 via `scipy.constants` (`nist.get_constant`).
2. **Hydrogen `2P₃/₂–2P₁/₂`** — the **theoretical/Ritz** level difference tabulated in the NIST ASD (`82\,259.2850014 − 82258.9191133` cm⁻¹, converted at `c`, via `nist.get_levels`), **not** an independent measurement. It must not be read as corroborating the framework; it differs from the measured `10\,969.13(10)` MHz by `~0.08` MHz.
3. **1S hyperfine, helium `^{3}P`, positronium ortho-para, muonium hyperfine** — served by `nist.list_dirac_targets`, a **curated** registry. These splittings are absent from the NIST ASD at this precision (and the PDG machine-readable API has no positronium/muonium entries), so the registry transcribes the values from this document's measurement column / primary literature. They are therefore retrievable via the MCP **but circular by construction** — they restate the `Measurement` column, they do not independently confirm it.

<!-- TODO: human reviews and fills in — confirms the MCP cross-check column is correctly framed: g_s is the only independent source (CODATA); the H fine-structure entry is ASD-theoretical (not a measurement); and the curated `nist.list_dirac_targets` entries (1S hfs, He 3P, positronium, muonium) are transcriptions of this doc's measurement column, retrievable for convenience but not independent corroboration -->
