...
**The Lamb shift is the campaign's strongest experimental endorsement available to date.** At the Bethe-estimate precision the proper-time framework can deliver, it reproduces measured behaviour to the same precision standard QED's Bethe-estimate route delivers. Pushing below the Bethe-estimate residual to full one-loop precision requires a proper-time one-loop dual-QED calculation that the framework has not yet produced.
