...
A further refinement is still open: a first-principles derivation of $r_e$ from the dual-Dirac renormalisation prescription (DRQM I §III.D) — tracked in [issue #54](https://github.com/temoTxt/PyPhysics/issues/54). The three candidate starting points sketched briefly below are expanded into 1–3 paragraph explanations each in a companion document, [`2026-05_re_derivation_candidates_for_gill.md`](2026-05_re_derivation_candidates_for_gill.md).
...
- a structural constant of the dual representation — something the $b$-factor projection structure or the second-order Dirac decomposition singles out — making $r_e/r_0$ a derivable framework-internal ratio rather than a renormalisation choice. (Per your 2026-05-25 author guidance, a fourth candidate we had initially considered — retrieving an unpublished working-notebook derivation step — is empty, since the original cutoff value was a numerical search against $g_s$ alone.)
