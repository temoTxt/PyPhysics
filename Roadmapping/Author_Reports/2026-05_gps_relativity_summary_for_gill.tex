...
by a popular-science article on the headline \(\pm 38\,\mu\)s/day
...
(2003)\footnote{Ashby, N. (2003). \emph{Relativity in the Global
  Positioning System.} Living Reviews in Relativity.}. The headline
numerical decomposition reproduces Ashby end-to-end at the precision the
published parameters can sustain.
...
The companion campaign\footnote{Proper-time companion campaign. PR \#73,
  \texttt{github.com/temoTxt/PyPhysics/pull/73}.} worked the same nine
effects under the Gill--Zachary substitution rules\footnote{Gill, T. L.
  and Zachary, W. W. \emph{Two Mathematically Equivalent Versions of
...
cleanest demonstration in the campaign is pt\_03 velocity time dilation.
We observe that the framework's identity

\[\frac{d\tau}{dt} = \frac{c}{b} = \sqrt{1 - \mathbf{w}^2/c^2} = \frac{1}{\gamma} \qquad (5)\]

is exact, by algebraic identity from the velocity-duality substitution
\(\mathbf{w}/c = \mathbf{u}/b\). The numerical value \(-7.11\,\mu\)s/day
is identical to the standard prediction at all decimal places, not at
any precision floor. For pt\_06 Sagnac, the rotating-frame photon-null
condition runs through with \(c \to b\) substitution. The result is
...
reduce to standard at GPS-relevant velocities. The combined headline of
...
\textbf{Refuted --- framework as-published doesn't extend (pt\_02,
...
result. Every document in this category carries a substantive-AI TODO
...
validate, either formulation. We observe that a negative finding --- the
framework predicting something at GPS precision that disagreed with
measurement --- would have been a serious problem for the framework.
None was found.
...
surface from the proper-time companion's Refuted and Marginal per-effect
documents. Each is operationally invisible at current-generation GPS
precision. Each is structurally load-bearing for the framework's
extension to regimes where the differences would matter. Each question
is one short paragraph by design. The per-effect documents under the
GPS-relativity folder\footnote{Per-effect documents.
...
element, with \(b \to c\) at low velocity. This is one possible
extension. We considered three others without resolution. The
alternatives are: (a) the substitution \(c \to b\) applied only in the
time-time metric component \(g_{00}\); (b) treating null and timelike
geodesics differently, since the photon's local frame has
\(\mathbf{u} = 0\) and so \(b = c\), but the worldline along which the
metric is evaluated does not; (c) replacing the metric structure
entirely with a \(b\)-dependent connection that reduces to the
Schwarzschild connection in the \(b = c\) limit. For GPS at the
\(10^{-10}\) precision floor, all four hypotheses reduce to the same
operational answer. The question matters for next-generation
...
enter the Sagnac formula on the source side as well. That reading would
predict an additional satellite-side correction at the \(\sim 10^{-10}\)
level of the standard Sagnac. This sits below current GPS precision, but
at the edge of next-generation optical-clock receivers. We ask whether
the framework intends the source-side and receiver-side \(b\) to enter
symmetrically, or whether the receiver-frame derivation is the complete
prediction.
...
standard \(+0.033\,\mu\)s/day contribution to the headline. A
...
\item
  ...
  partially dependent on it. Q2 is independent and propagates to the
  ...
  the four Refuted and Marginal per-effect documents from speculative to
  derived.
\item
  ...
  radiation-reaction sub-investigation\footnote{Radiation-reaction
    sub-investigation. Issue \#43,
    \texttt{github.com/temoTxt/PyPhysics/issues/43}.}. Since the
...
\item
  ...
  clock-stability floor. If you signal interest in either, the campaign
  would open follow-on threads.
\end{enumerate}
...
proper-time photon propagator --- that is the natural next campaign. We
would scope it as a follow-on issue keyed off your direction. If the
...
We thank you for the opening to send focused follow-ups in this form.
...
the highest-load ask within the GPS scope, but is a much smaller
decision since GPS is operationally non-falsifying at current precision.
