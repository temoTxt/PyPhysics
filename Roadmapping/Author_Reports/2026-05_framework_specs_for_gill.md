---
title: "Three framework-specification questions for a first-principles \\(r_e\\) derivation"
...
We present here three framework-specification questions that have proved necessary across the three parallel candidate investigations of a first-principles derivation of $r_e/r_0$. Each question, if resolved, would permit a derivation that goes beyond the algebraic inheritance from QED's $a_e(\alpha)$ that the present closed-form result expresses.

## What the three investigations converged on
...
is necessary but not sufficient for intentional Schwinger encoding. A cross-particle test against the FNAL Muon $g-2$ 2023 measurement, performed in the same investigation, yielded
...
Candidate 2 found that the published expanded $K_D$ of Eq. (III.4) is structurally inadequate to pin $r_e$ via the variational principle[^6]. No scale exists where both (a) the NR expansion is valid and (b) the cutoff couples meaningfully to the closure equation. At the electron-radius scale ($\hat r_e \sim 1$) the kinetic-energy term dominates by four orders of magnitude; the NR expansion is therefore invalid. At the Bohr scale the expansion is valid, but the cutoff $\hat r_e$ is suppressed by $\alpha^2$, and the closure matches plain hydrogen to ten significant figures.
...
matching the triangulated value to $3.45 \times 10^{-13}$[^7]. Equation (3) is the algebraic inverse of the framework's existing $g$-formula (Eq. III.22) evaluated at QED's $a_e(\alpha)$. This corresponds to reproduction-by-inheritance under hypothesis (ii), in which the photon propagator is unchanged and all dual structure is absorbed into the (II.3) Pauli source. We observe that, on this reading, the framework does not independently re-derive Schwinger's vertex result; it inherits it.

The convergent gap across all three candidates is a first-principles derivation of $a_e$ from framework-internal dynamics. Such a derivation requires three framework specifications that the published apparatus does not currently supply. We do not ask the authors to perform the derivation; rather, we ask whether the framework, as intended, specifies (or implicitly fixes) the three pieces below. Each admits a brief answer.
...
with the $b/c$ conversion absorbed into the source's coupling rather than into the propagator, and the dual structure entering through modified vertex factors. The two readings give different numerical $r_e/r_0$ at the same order in $\alpha$. At the $\log(b/c)$ level they are discriminable at the framework's precision. It is not clear to us which (if either) is the intended choice.

### 2. The bound-state propagator $G_C(x,y;E)$ in the (II.3) "potential-in-the-mass" kernel

For the bound-electron one-loop self-energy, one requires a propagator that handles the Coulomb potential non-perturbatively at the cutoff scale, where the NR expansion of Eq. (III.4) is invalid (per Candidate 2's diagnostic). Standard QED uses the Furry-picture Coulomb-bound propagator, summing the external Coulomb potential to all orders in $Z\alpha$ before perturbing in $\alpha$. It is not clear from the published apparatus whether the framework specifies (or implicitly uses) a dual analog of the Furry-picture construction, or some other treatment of the bound-state propagator under the (II.3) kernel.
...
The textbook NR working hypothesis ($\Delta E_{\rm bind} = \langle V_0\rangle$, $\Delta E_{\rm SE} = 0$) gives a trivially-zero closure under the published expanded $K_D$, per Candidate 2's diagnostic. We seek the framework's intended form for $\Delta E_{\rm SE}^{\rm framework}(r_e)$ at the cutoff scale. The relevant analog is QED's on-shell mass renormalisation condition,
...
Any one of these would advance the corresponding candidate; all three together would permit the full hypothesis-(i) calculation. The thread is non-urgent. The triangulated value
...
already serves as the campaign's current best refinement at the framework's precision floor[^2]. It is consistent with the framework's apparatus at every observable in the joint fit. The substantive content of the three questions is whether the framework, as intended, contains the specifications that would close the gap from empirical cutoff to framework-derived quantity. The alternative is that closing that gap is genuinely new theory work the published apparatus does not yet support.

If the answer to any of (1), (2), (3) is that the framework does not yet specify the relevant piece, that is itself a substantive result. It would mean the campaign's terminal verdict on $r_e/r_0$ should be recorded as **Branch B, QED-inheritance, pending future framework development**, rather than as **Branch A, from first principles**. Either outcome closes the question honestly; what is at stake here is which honest framing is the right one.

The corresponding responses will be recorded on the framework-specifications issue[^3], and the derivation work resumed once the specifications are in hand.
