...
3. **`r_0` critical-point claim on line 256.** ✅ **Resolved 2026-05-11.** *FOUNDATIONS FOR QED II: CLASSICAL* Sec 2.2.1 supplies the explicit derivation. The result holds in the framework where the Hamiltonian has the "V as part of the mass" form $H = \sqrt{c^2\mathbf{p}^2 + (mc^2+V)^2}$ (not the standard "V as additive" form). Then $\mathbf{F}_K = -\nabla V(1 + V/(mc^2))$ has a critical point at $V = -mc^2$, which for $V = -e^2/r$ corresponds to $r = r_0 = e^2/(mc^2)$. Wolfram MCP confirms repulsive/zero/attractive sign for $r < r_0$, $r = r_0$, $r > r_0$. See [`FoundationsII-Classical.md`](./FoundationsII-Classical.md).
