...
**Update — 2026-05-26 iter-5/6: closed-form expression by (III.22) inversion (closes [#64](https://github.com/temoTxt/PyPhysics/issues/64)).** The candidate-1 work — originally scoped as a "proper-time self-energy integral derivation" on branch `64-theory-candidate-1-proper-time-self-energy-integral-derivation-of-r_e` — pivoted at iter-5 (see STATE.md) when iters 1-4 confirmed that no proper-time photon propagator exists in the verified corpus. The iter-5 deliverable is *not* a first-principles dual derivation; it is an algebraic inversion of the (III.22) formula combined with textbook QED $a_e$ inherited under hypothesis (ii). Recorded in full at [`Dual_Relativistic_Quantum_Mechanics_I.md §III.D-extension`](Dual_Relativistic_Quantum_Mechanics_I.md#iiid-extension--closed-form-r_er_0-from-iii22-inversion-closes-64) with companion notebook [`../Mathematica_Notebooks/Quantum_Mechanics/r_e_derivation_self_energy.wl`](../Mathematica_Notebooks/Quantum_Mechanics/r_e_derivation_self_energy.wl) (Cells 2–4 contain the actual derivation; Cell 1 + the trailing Bethe-log heuristic are vestigial scaffolding from iter-1-4 and are flagged as such).
...
(Wolfram MCP `FullSimplify[...] = 0`.) At one-loop, $a_e^{(1)} = \alpha/(2\pi)$ (Schwinger 1948, standard QED), so the closed form evaluates to $(2-\alpha/(2\pi))/(4+\alpha/\pi) = 0.499\,419\,632\,156$. This is the 1-loop sub-result of the framework's closed form — *not* a result Schwinger derived. (Issue #64 refers to this number as the "Schwinger closed-form reference"; that naming conflates "Schwinger's a_e" with "the (III.22)-inverted r_e at Schwinger's a_e".)
...
The CODATA-full residual $3.45\times 10^{-13}$ sits **1.4σ above** the triangulation precision floor $\sigma_r = 2.50\times 10^{-13}$ — consistent at 2σ, not at 1σ. Wolfram MCP confirms `Abs[residual] > sigma → True`, `residual/sigma = 1.38`. The agreement is to within an order of magnitude of the precision floor; the closed form is *consistent with* the triangulation at framework precision without strictly *attaining* it at 1σ.

**Hypothesis (ii) caveat (Crocco rule #5 — substantive AI honest scope).** The result is *reproduction by inheritance*, not an independent dual-framework derivation. Three honest qualifiers:

1. The closed form is an *algebraic inversion* of the framework's (III.22) cutoff formula, not a first-principles derivation of $r_e$ from the §III.7 physical argument (small-component-elimination breakdown). It tells you the value of $r_e$ that reproduces measured $g_e$ via (III.22); whether the framework's physical argument independently *predicts* that value is a separate question this PR does not address.
2. $a_e$ is *inherited* from textbook QED under hypothesis (ii) (photon propagator unchanged; dual structure absorbed into the (II.3) "potential-in-the-mass" kernel), at the precision the (II.3) kernel can deliver. This is the magnetic-moment-route analogue of the BS-§19 / §20 Lamb-shift inheritance argument, applied to the route where $r_e$ *does* engage (the Lamb shift route, BS-§20 line 114, does not).
3. The framework's substantive contribution is the (III.22) formula itself — the identification of the cutoff radius $r_e$ with the anomalous magnetic moment via $g_r(r_e/r_0) = -2(1+a_e)$. Given that identification, the closed form $r_e/r_0 = (2-a_e)/(2(2+a_e))$ follows from one-step algebra, and the precision is set by the QED precision in $a_e$.
