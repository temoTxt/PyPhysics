...
reads (PDF with LaTeX equations, not Mathematica notebook).
The substantive-AI selection is which subset of the chain to surface for a
review pass; the equations themselves are transcribed verbatim from
DRQM~I.}
...
\paragraph{The chain-rule step in (III.6).}
Substituting~\eqref{eq:III2} into the $\psi_1$~equation produces a
non-trivial chain-rule term because $\boldsymbol\sigma\!\cdot\!\boldsymbol\pi$
does not commute with $1/(\lambda - V_0 + m c^2)$:
\begin{equation}
\label{eq:III6-chain}
...
(\boldsymbol\sigma\!\cdot\!\boldsymbol\pi)\,\psi_1.
\end{equation}
The second term is the source of $(V_0/m)\,(\boldsymbol\sigma\!\cdot\!\mathbf{p}V_0)
(\boldsymbol\sigma\!\cdot\!\boldsymbol\pi)/(\lambda - V_0 + m c^2)^2\,\psi$
in DRQM~I line~261.

\paragraph{Cutoff approximation (III.7).}
...
After the Pauli identity collapse and the spherical-coordinate algebra
(DRQM~I Eqs.~(III.9)--(III.17)), three contributions appear in the
approximated eigenvalue equation~(III.8) that are not present in the
non-relativistic Schr\"odinger problem:
\begin{align}
...
the algebraic inversion). Unconditional ✅ requires hypothesis-(i),
issue~\#75.
...
laid out in DRQM~I \S\S II--III. The substantive AI selections are:
(1)~which subset of (III.5)--(III.20) to surface in the PDF (the chain-rule
step in (III.6), the cutoff approximation in (III.7), and the three
(III.18)--(III.20) ``new terms'' are kept; the spherical-coordinate
algebra (III.11)--(III.17) is summarised); (2)~the per-observable $n_i$
column in the summary table (drawn from the BS section docs but consolidated
here for the first time in one place). Please confirm both choices
match your intent for a Tepper-review presentation.}
