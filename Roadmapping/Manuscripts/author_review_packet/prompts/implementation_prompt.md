...
3. *(after second build)*: "1. surface all the equations for fine-grain review. 2 keep the table and approve the synthesis move"

   → Expanded [`sections/02a_derivation_chain.tex`](../sections/02a_derivation_chain.tex) so every numbered DRQM I equation (II.1)–(III.22) carrying a typeset form in the verification doc is surfaced as a displayed equation. Inserted: explicit (III.4) expanded form, (III.5)-(III.6) spinor split + chain rule (now in correct position after (III.3)), (III.8) approximated $\psi_1$-only eigenvalue, (III.9)-(III.10) Pauli identity, and (III.11)-(III.17) spherical-coordinate algebra (the three load-bearing outputs as displayed equations, with an honest note that (III.11)-(III.16) intermediate vector-calculus steps are "mechanical algebra" per the verification doc itself rather than re-derived here). The per-observable $n_i$ summary table is endorsed and remains as the only substantive synthesis move beyond transcription.
